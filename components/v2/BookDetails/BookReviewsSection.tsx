...
import { type BookRatingsProps, starLabels } from "const";
...
export default function BookReviewsSection() {
	...
	switch (bookRatingLoadable.state) {
		case "hasValue":
			...
			return (
				<>
					<div className="justify-start h-auto mt-6 hero">
						<div className="items-start hero-content">
...
const StarPercentageBar = (props: { leftText?: string; value: number }) => {
	...
	return (
		<div className="flex items-center justify-between gap-2">
			{leftText && (
				<span className="w-32 text-sm text-gray-500">{leftText}</span>
			)}
			<progress
				...
				max="100"
			></progress>
			<span className="w-32 text-sm text-gray-500">{`${valueRound}%`}</span>
		</div>
	);
};
									...
									<td>
										<div className="flex items-center space-x-3">
											<div className="avatar placeholder">
												<div className="w-12 h-12 bg-neutral-focus text-neutral-content mask mask-squircle">
